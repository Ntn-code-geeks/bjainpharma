...
//             pr($result); die;
	...
	$this->db->where('stp.dot  <=', $end);
      ...
      public function save_tada_report($data){
        // pr($data);
        // echo ''; die;
		...
		$this->db->insert('genrated_ta_da_master',$genrated_ta_da_master_data); 
                ...
                if($this->db->affected_rows() == 1){
                    ...
                    foreach($data->tadaData as $k_tada=>$val_tada){
                        
                          $genrated_ta_da_detail_data = array(
                                    'ta_da_master_id'=>$last_id,
                                    'ineraction_date'=> strtotime($val_tada->doi),
                                    'from_city'=>$val_tada->source_city,
                                    'city_worked'=>$val_tada->destination_city,
                                    'distance'=>$val_tada->distance,
                                    'ta'=>$val_tada->ta,
                                    'da'=>$val_tada->da,
                                    'postage'=>$val_tada->postage,
                              
                                    'any_charge_name'=>isset($val_tada->chargeName)?$val_tada->chargeName:NULL,
                                    'any_charge_amount'=>isset($val_tada->chargeAmount)?$val_tada->chargeAmount:NULL,
                              
                                    'filled_date'=>strtotime($val_tada->create_date),
                                    'back_ho'=>$val_tada->back_ho,
                              
                                    'total_amount'=>$val_tada->total,
