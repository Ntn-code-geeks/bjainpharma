...
//						'total_distance'=> $cityDistance,
	...
	public function save_tour_data($data){
		$tour_data = array(
			'source_city'=> isset($data['city_from'])?$data['city_from']:0,
			'dist_city'=>$data['city_to'],
//			'fare'=>$data['city_fare'],
			'distance'=> isset($data['city_distance'])?$data['city_distance']:0,
...
//            pr($data); die;
		$tour_data = array(
			...
			'dist_city'=>$data['city_to'],
                        ...
                        'is_approved'=> isset($data['approve_submit'])?1:0,
//			'fare'=>$data['city_fare'],
			'distance'=> isset($data['city_distance'])?$data['city_distance']:0,
			'pharma_user_id'=>$data['user_id'],
			'remark'=>$data['remark'],
	...
	public function get_tour_dest($data)
	{
        ...
        /*
         * Developer: Shailesh Saraaswat
         * Email: sss.shailesh@gmail.com
         * Dated: 05-JAN-2018
         * 
         * get child parent of a logged user
         * 
         */
        
        public function child_parent($users){
            
            $arr = "name,id as userid,user_designation_id";
            $this->db->select($arr);
            $this->db->from('pharma_users');
      ...
      * Dated: 15-JAN-2019
      * 
      * check that user have tour plan on that date or not
      * 
      */   
        
     public function user_stp_details($userids,$plan_date){
         
          $arr = "remark,crm_user_id as userid,id as stp_id";
            $this->db->select($arr);
            $this->db->from('user_stp');
            
            $this->db->where('crm_user_id',$userids[0]['userid']);
            $this->db->where('dot',date('Y-m-d',strtotime($plan_date)));
            
            $this->db->group_by('dot');
...
//               echo $this->db->last_query(); die;
     ...
     public function change_tour_plan($request_id){
         
          $change_tpdp  = $this->user_requested_data($request_id);
         
           $data = array(
                    'crm_user_id'=>$change_tpdp->person_id,
                    'destination'=>$change_tpdp->city_id,
                    'dot'=>$change_tpdp->tour_date,
                    'assign_by'=>$change_tpdp->assign_by,
                    'status'=>1,
                    'color_id'=>'#f56954',           
                    'updated_date'=> savedate(),
                );
           
         $this->db->where('crm_user_id',$change_tpdp->person_id);
         $this->db->where('dot',date('Y-m-d',strtotime($change_tpdp->tour_date)));
         
         $this->db->update('user_stp',$data);
     ...
     private function user_requested_data($request_id){
         
         $arr = 'person_id,tour_date,city_id,crm_user_id as assign_by';
         
          $this->db->select($arr);
            $this->db->from('tp_change_request');
...
//               echo $this->db->last_query(); die;
            ...
            if($this->db->affected_rows())
                {
                    ...
                    $this->db->update('tp_change_request',$change_requested);
     ...
     public function city_change_request($person,$tour_date,$city){
         ...
         $this->db->insert('tp_change_request',$data);
