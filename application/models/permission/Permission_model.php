...
public function __construct()
    ...
    public function user_list($id=''){
        ...
        $this->db->join("city c","c.city_id=pu.hq_city",'left');
...
//     pr($child_id); die;
    ...
    if(!empty($boss)){
         $bosswhere = " ( "; $total_boss = count($boss_id);
      foreach($boss_id as $kb=>$valb){
           $bosswhere .="pu.id = '$valb'";
    ...
    if(!empty($child)){
         ...
         if(!empty($boss)){
             $bosswhere .= " OR ";
                ...
                $total_child = count($child_id);
      foreach($child_id as $kc=>$valc){
            $bosswhere .="pu.id = '$valc'";
...
//     echo $this->db->last_query(); die;
   ...
   * 
   */
  public function user_child_team(){
