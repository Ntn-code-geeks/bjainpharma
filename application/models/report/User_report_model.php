...
$leave_dy=count($leaveSunday)-count($sundayDate);
  $absent = count(array_diff($leaveNotApply, $holidayDate));
...
public function check_leave_holiday($idate,$userid){
		$holiday = array(); $leave_time = array();
        ...
        if( empty($dealerIntearction) && empty($pharmaIntearction) && empty($docInteraction) && empty($holiday) && empty($leave_time) ) {
        		return 3;
        }
        else{
              return 1;
