...
//                                    'manager_id' => $boss_ID[0]['boss_id'],
		...
		$this->db->insert('genrated_ta_da_master',$genrated_ta_da_master_data); 
                
                $last_id = $this->db->insert_id();
                if($this->db->affected_rows() == 1){
                    
                    foreach($data['doi'] as $k_tada=>$val_tada){
                        
                          $genrated_ta_da_detail_data = array(
                                    'ta_da_master_id'=>$last_id,
                                    // 'ineraction_date'=> strtotime($val_tada),
                                    'ineraction_date'=> $val_tada,
                                    'from_city'=>$data['source_city'][$k_tada],
                                    'city_worked'=>$data['destination_city'][$k_tada],
                                    'distance'=>$data['distance'][$k_tada],
                                    'ta'=>$data['ta'][$k_tada],
                                    'da'=>$data['da'][$k_tada],
                                    'postage'=>$data['internet_charge'][$k_tada],
                              
                                    'any_charge_name'=>$data['aoc_name'][$k_tada],
                                    'any_charge_amount'=>$data['aoc_amount'][$k_tada],
                              
                                    'filled_date'=>strtotime($data['create_date'][$k_tada]),
                                    'back_ho'=>$data['back_ho'][$k_tada],
                              
                                    'total_amount'=>$data['totalrow'][$k_tada],
    ...
    // save manager tada approved report
    
     public function save_tada_approved_report_of_manager($data,$tada_id){
//        pr($data);
//        echo $tada_id; die;
        $genrated_ta_da_master_data = array(
...
//                                  'create_date'=>savedate(),
         ...
         public function save_tada_approved_report_of_admin($data,$tada_id){
//        pr($data);
//        echo $tada_id; die;
        $genrated_ta_da_master_data = array(
//            			'admin_total_amount'=>$data['admin_grant_total'],
		...
		$this->db->update('genrated_ta_da_master',$genrated_ta_da_master_data); 
                ...
                if($this->db->affected_rows() == 1){
                    ...
                    foreach($data['admin_remark'] as $k_tada=>$val_tada){
...
//                                    'create_date'=>savedate(),                  
    ...
    // check that a user must genrate your ta/da report first tiem only
    public function first_time_genrate($monthyear,$userid){
        
        $this->db->select('id');
        $this->db->from('genrated_ta_da_master');
    ...
    // select tada report list of a user
    public function tada_report_list(){
        
      $child = logged_user_child();
      $child_id = explode(',', $child);
      
        $arr="pu.name,ta.month_year,ta.total_amount as grant_total,ta.manager_total_amount,"
                . "ta.manager_id,ta.admin_id,ta.admin_total_amount,ta.id";
        $this->db->select($arr);
        $this->db->from('genrated_ta_da_master ta');
        $this->db->join('pharma_users pu','ta.user_id=pu.id','left');  
        
        if(is_admin()){
             $this->db->where('ta.manager_id is NOT NULL', NULL, FALSE); 
        }
        
        if(!empty($child) && !is_admin()){
          foreach($child_id as $kc=>$valc){
            $this->db->or_where('ta.user_id',$valc);
          }
        }
        
        
        if(!is_admin() && empty($child)){
            $this->db->where('ta.user_id', logged_user_data());  
        }
...
//         echo $this->db->last_query(); die;
    ...
    // get SE genrated ta da report 
    public function genrated_tada_report($tada_id){
        
        $arr="ineraction_date,from_city,city_worked,distance,ta,da,"
                 . "postage,total_amount,manager_remark,admin_remark,id as tada_detail_id,"
                 . "any_charge_name as aoc_name,any_charge_amount as aoc_amount ";
        $this->db->select($arr);
        $this->db->from('genrated_ta_da_details');
        $this->db->where('ta_da_master_id',$tada_id);
...
//         echo $this->db->last_query(); die;
     ...
     public function verified_admin_tada_report($userid,$month_year){
        ...
        if(is_admin()){
             $this->db->where('ta.manager_id is NOT NULL', NULL, FALSE); 
             $this->db->where('ta.admin_id is NOT NULL', NULL, FALSE); 
        }
...
//                pr($result); die;
                $tada_id = $result[0]['id'];
                 ...
                 $arr="ineraction_date,from_city,city_worked,distance,ta,da,"
                 . "postage,total_amount,manager_remark,admin_remark,id as tada_detail_id,"
                 . "any_charge_name as aoc_name,any_charge_amount as aoc_amount,filled_date,back_ho ";
                    $this->db->select($arr);
                    $this->db->from('genrated_ta_da_details');
                    $this->db->where('ta_da_master_id',$tada_id);
            ...
            //         echo $this->db->last_query(); die;
                    if($this->db->affected_rows()){
                             $tada_details =  $query->result_array();
                        
                             $result['tada_detail'] = $tada_details;
                      return $result;
                    }
                    else{
                      return FALSE;
