                ...
                <div class="row tab-pane" id="changecity" style="margin: auto; width: auto; ">
                    <form name="changeloca" method="post" action="<?= base_url();?>interaction/add_direct_inteaction">
                        <label style="padding: 60px 40px 60px; margin-bottom: 50px;">
                            Interaction In Another City:
                        </label>
                    <select name="an_interaction_city" id="an_interaction_city"  class="form-control select2"
                            style="width:60%;  margin: auto;" >
                        <option value="">--Select City--</option>
                    <?php
                    $userID=logged_user_data();
                    $spcode=getuserSPcode($userID);
                    $city_list=get_all_city($this->session->userdata('sp_code'));
                    foreach($city_list as $city){ ?>
                        <option value="<?=$city['city_id']?>" ><?=$city['city_name'].'('.$city['state_name'].')'?></option>
                    <?php }  ?>
                    </select>
                    <input type="hidden" name="date_interact" id="date_interact" value="<?=$date_interact; ?>">
<!--                    <div id="selectvalue"></div>-->
                        <button type="submit" name="changeloc"  id="changeloc" class="btn
