...
class Interaction extends REST_Controller {
    ...
    // Construct the parent class
       ...
       function asm_interaction_report_post()
    {
//          echo 'hello'; die;
        # initialize variables
	$msg = '';
	$asm_data=json_decode($this->input->raw_input_stream);
//       pr($asm_data); die;
	if(!isset ($asm_data->selectInteractionDate) || empty($asm_data->selectInteractionDate))
	{
		$msg = 'Please Enter Interation date.';
	}
	elseif(!check_leave($asm_data->selectInteractionDate,$asm_data->user_id))
	{
		$msg = 'You have taken leave  or holiday on that day please change date!!';
	}
	elseif(!isset ($asm_data->cityId) || empty($asm_data->cityId))
	{
	   $msg = 'Please Enter City';
	}

	elseif(!isset ($asm_data->bossId) || empty($asm_data->bossId))  // this is a child id
	{
	   $msg = 'Please Enter Child Id';
	}
        elseif(!check_interaction_repeat($asm_data->selectInteractionDate,$asm_data->user_id,$asm_data->cityId,$asm_data->bossId))
	{
		$msg = 'You Already Submit the interaction on same date!!';
	}
//        echo $msg;
//	die;
       	if ($msg == '') 
        {
	        $data = $this->interact->save_asm_dsr($asm_data);
		if ($data!=FALSE) 
		{ 
	            $result = array(
	                'Data' => $data,
	        ...
	        else 
	        {
	            $result = array(
	                'Data' => new stdClass(),
	                'Status' => false,
	                'Message' => 'Error in Save Interaction info.',
	                'Code' => 404
	            );
	        }
        }
		...
		if($docdata!=FALSE)				{
          ...
          if(!isset($interaction_data->totalSale)){
              if ($interaction_data->interactionType==1){
                ...
                <div class="regards_box"><h3 class="user_name_regards">Regards,</h3><p class="user_name_brand">'.get_user_name($interaction_data->user_id).'<br>B. Jain Pharmaceuticals Pvt. Ltd.</p></div></div></body></html>';
              }
              else{
