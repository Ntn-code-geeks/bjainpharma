...
class Pharma extends REST_Controller {
	...
	$sp_code  = $post['sp_code'];
        if(!(isset($sp_code)))
        {
       	...
       	if ($msg == '') 
        {
	        $data = $this->pharma->get_pharma_list($sp_code);
		if ($data!=FALSE) 
		{ 
	            $result = array(
	                'Data' => $data,
     ...
     * Dated: 04-JAN-2019
     * 
     * Get pharma list on the basis of pincode
     */
    function pharma_list_v2_post()
    {
        # initialize variables
        $post = array_map('trim', $this->input->post());
	$msg = '';
	$pin_code  = $post['pincode'];
        ...
        if(!(isset($pin_code)))
        {
       	...
       	if ($msg == '') 
        {
	        $data = $this->pharma->get_pharma_list($sp_code,$pin_code);
		if ($data!=FALSE) 
		{ 
	            $result = array(
	                'Data' => $data,
