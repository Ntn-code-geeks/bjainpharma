       	...
       	if ($msg == '') 
        {
            ...
            if(((isset($post['overall_secondary_report'])) && ($post['overall_secondary_report'] == 1)) ){
                ...
                if($timeline==''){
                    ...
                    $jsonData = file_get_contents ("ReportJSON/weekly.json");
                    ...
                    $jsonData = file_get_contents ("ReportJSON/monthly.json");
                    $json = json_decode($jsonData, true);
                    foreach($json as $user) {
                        if (in_array($user['user_id'], $childs_users)) {
                            if($user['total_doc_interaction'] > 0){
                    ...
                    $jsonData = file_get_contents ("ReportJSON/quarterly.json");
                    $json = json_decode($jsonData, true);
                    foreach($json as $user) {
                        if (in_array($user['user_id'], $childs_users)) {
                            if($user['total_doc_interaction'] > 0){
                    ...
                    $jsonData = file_get_contents ("ReportJSON/yearly.json");
                    $json = json_decode($jsonData, true);
                    foreach($json as $user) {
                        if (in_array($user['user_id'], $childs_users)) {
                            if($user['total_doc_interaction'] > 0){
