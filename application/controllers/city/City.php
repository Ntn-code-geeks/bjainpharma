...
class City extends Parent_admin_controller {
	...
	/*
         * Date: 24-04-2019
         * Downlaod all city with code
         * 
         */
        public function createXLS() {
//                  $this->load->library('excel');
		// create file name
        $fileName = 'data-city'.time().'.csv';  
		// load excel library
        $this->load->library('excel');
        $cityInfo = $this->city->get_city_list();
//        pr($cityInfo); die;
        $objPHPExcel = new PHPExcel();
        $objPHPExcel->setActiveSheetIndex(0);
        // set Header
        $objPHPExcel->getActiveSheet()->SetCellValue('A1', 'City Code');
        $objPHPExcel->getActiveSheet()->SetCellValue('B1', 'City Name');
        $objPHPExcel->getActiveSheet()->SetCellValue('C1', 'State Code');
        $objPHPExcel->getActiveSheet()->SetCellValue('D1', 'State Name');
        $objPHPExcel->getActiveSheet()->SetCellValue('E1', 'Is Metro');
        $objPHPExcel->getActiveSheet()->SetCellValue('F1', 'Pool PinCode');       
        // set Row
        $rowCount = 2;
        foreach ($cityInfo as $element) {
            $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, $element['city_id']);
            $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $element['city_name']);
            $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $element['state_id']);
            $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $element['state_name']);
            $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $element['is_metro']);
				...
				while (($importdata = fgetcsv($file, 1000000, ",")) !== FALSE)
				{
					...
					$data = array(
						...
						'city_name' =>strtoupper($importdata[1]),
                                                'is_metro'=>$importdata[2],
                                                'pool_pincode'=>$importdata[3],
						'status' =>1,
