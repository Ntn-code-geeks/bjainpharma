...
//echo $this->db->last_query(); die;
	...
	public function save_interaction($data){
		...
		if(!empty($data->doc_id)){   // for Doctor interaction
			...
			if(!empty($data->m_sample)){  // for multipile sample data
				if(!empty($data->m_sale)){
					$interaction_info = array(
						'doc_id'=>$data->doc_id,
						'dealer_id'=>!empty($dat->dealer_id)?$data->dealer_id:NULL,
...
//                echo $insert; die;
				...
				if(!empty($pi_doc)){
					...
					$order_data = array(
						...
						'interaction_with_id'=>!empty($data->dealer_id)?$data->dealer_id:NULL,
				...
				if(!empty($pi_doc) && !empty($data->team_member)){
				...
				if(!empty($data->m_sample) && !empty($data->team_member)){
				...
				elseif(!empty($data->m_sample) && empty($data->team_member)){
				...
				if(!empty($data->m_sale)){
					$interaction_info = array(
						'doc_id'=>$data->doc_id,
						'dealer_id'=>!empty($data->dealer_id)?$data->dealer_id:NULL,
						'meeting_sale'=>$data->m_sale,
						'telephonic' => !empty($data->telephonic)? $data->telephonic:NULL,
						'meet_or_not_meet'=>$meet_or_not,
						'remark'=>$data->remark,
						'follow_up_action'=>$data->fup_a!='' ? date('Y-m-d', strtotime($data->fup_a)):NULL,
						'status'=>1,
...
//           echo $this->db->last_query(); die;
		...
		else if(!empty($data->d_id)){   // for dealer interaction
		// else if(isset($data->d_id)){   // for dealer interaction
			if(!empty($data->m_sample)){  // for multipile sample data
				$interaction_info = array(
					'd_id'=>$data->d_id,
					'meeting_sale'=>!empty($data->m_sale)? $data->m_sale:NULL,
				...
				if(!empty($pi_dealer) && !empty($data->team_member)){
					foreach($data->team_member as $k_tm=>$val_tm){
		...
		elseif(!empty($data->pharma_id)){   // for pharmacy interaction
		// elseif(isset($data->pharma_id)){   // for pharmacy interaction
			$dup_count=0;
			$dup_product='';
			if(!empty($data->rel_doc_id))
			{
			...
			if(!empty($data->m_sample)){  // for multipile sample data
				if(!empty($data->m_sale)){
					$interaction_info = array(
						...
						'dealer_id'=>!empty($data->dealer_id)?$data->dealer_id:NULL,
						'doctor_id'=>!empty($data->rel_doc_id)?$data->rel_doc_id:NULL,
