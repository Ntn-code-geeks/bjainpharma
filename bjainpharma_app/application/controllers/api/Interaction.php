...
//pr($orderDetails); die;
        ...
        <tbody>' . $emailordt . '</tbody></table> ';
					 ...
					 if ((isset($interaction_data->telephonic)) || (isset($interaction_data->meet_or_not)) ) {
						 ...
						 $result_leave = $this->interact->checkleave($interactionDate,$user_id);
						 if(!$result_leave) {
							 $result = array(
							 'Data' => new stdClass(),
...
//						 return "Alert! Add Secondary / Add Product is Mandatory after  - Order Received.";
					 ...
					 if (!empty($interaction_data->m_sale) || !empty($interaction_data->m_payment) || !empty
						 ...
						 $success = $this->interact->save_interaction($interaction_data);
						 if ($success == 1) {
							 ...
							 if (isset($interaction_data->doc_name)) {
								 ...
								 if (isset($interaction_data->dealer_id)) {
									 if (is_numeric($interaction_data->dealer_id)) {
										 //for dealer;
										 $data = $this->interact->get_dealer_data($interaction_data->dealer_id);
										 if ($data != FALSE) {
										 ...
										 //for pharmacy;
										 $data = $this->interact->get_pharmacy_data($interaction_data->dealer_id);
										 if ($data != FALSE) {
								 ...
								 $docdata = $this->interact->get_doctor_data($interaction_data->doc_id);
								 if ($docdata != FALSE) {
...
//									 return "Alert! Something went wrong.";
							 ...
							 elseif (isset($interaction_data->pharma_id)) {
								 ...
								 $dataPharmacy = $this->interact->get_pharmacy_data($interaction_data->pharma_id);
...
//									 return "Alert! Something went wrong.";
							 ...
							 elseif (isset($interaction_data->d_id)) {            //for Dealer side
								 $data = $this->interact->get_dealer_data($interaction_data->d_id);
								 if ($data != FALSE) {
									 $dealerNumber = $data->d_phone;
									 $dealerEmail = $data->d_email;
								 }
...
//										 return "Success! Interaction is being saved for this.";
							 ...
							 $gd_id = json_decode($this->interact->edit_dealer($interaction_data->dealer_view_id));
									 if ($gd_id->gd_id == '') {
										 $result = array(
											 'Data' => $gd_id,
											 'Message' => 'Success! Interaction is being saved for this.',
	 ...
	 function joint_interaction_post()
	 {
		 ...
		 if ($msg == '') {
			 ...
			 if ($interaction_data->stay || $interaction_data->up) {
				 $success = $this->interact->save_asm_dsr($interaction_data);
