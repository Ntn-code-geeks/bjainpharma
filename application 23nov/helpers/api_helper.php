<?php
	...
	/*check for double entry of asm interaction of same city on same date*/
 function check_interaction_repeat($idate,$user_id,$city_id,$childid){
     $ci = &get_instance();
        $ci->db->select('interaction_id');
	$ci->db->from('asm_interaction');
	$ci->db->where('crm_user_id',$user_id);
        $ci->db->where('joint_work_with',$childid);
        $ci->db->where('interaction_city',$city_id);
	$ci->db->where('interaction_date', strtotime($idate));
	$query = $ci->db->get();
	...
	function get_assign_task_by($userid,$fromdate,$todate)
	{
	  ...
	  function get_followup_data($userid,$fromdate,$todate)
	  {
	    ...
	    while($fromdate<=$todate)
	    {
		    ...
		    if($remark!='')
		    {
		    	$followup[]=array('date'=>$fromdate,'remark'=>$remark) ;
		    }
