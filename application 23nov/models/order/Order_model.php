...
class Order_model extends CI_Model {
 ...
 public function products_list($data){
        $this->db->select('*');
        $this->db->from("pharma_product");
        $this->db->where('product_category',$data['catid']);
        $this->db->where('status',1);
        $queryDoc = $this->db->get();
        //echo $this->db->last_query(); die;
        if($this->db->affected_rows()){
            return json_encode($queryDoc->result_array());
        }
        else{
            return FALSE;
        }
    ...
    public function products_list_get($data){
        $this->db->select('*');
        $this->db->from("pharma_product");
...
//         pr($data); die;
	...
	public function get_cat_product($data){
        ...
        if($data['catid']==11 || $data['catid']==7){    
      	$arr = "product.product_id,product.product_name,product.product_category,"
                . "product.product_potency,product.product_packsize,"
                . "product.product_price,product.status,packsize.packsize_value";
        $this->db->select($arr);
        $this->db->from("pharma_product as product");       
		$this->db->join("pharma_packsize as packsize",'product.product_packsize=packsize.packsize_id');
		...
		$this->db->where('product.status',1);
                $this->db->group_by('product.product_price');
